...
* **代币搬运 (Scan & Sweep)**：支持「扫描资产」发现 ERC20（包含恶意代币/钓鱼币识别）。填写转出钱包私钥、Gas 赞助商私钥（必填）、代币接收地址（必填）。支持一键全部搬运 (Batch Sweep)，由赞助商代付 Gas 调用操作账户的 `sweepTokenTo(token, 接收地址)`。
* **部署合约 & 记录**：在支持的链上部署委托合约，并本地保存部署记录，支持按钱包过滤。
